...
suicide$status <- as.factor(suicide$Descript)
str(suicide$status)
head(suicide$status)
...
suicide$status <- ifelse(suicide$status %in% levels(suicide$status)[1:9],
...
sf_map+geom_point(aes(x =X,y =Y, color=status), data = suicide, 
                  size = .9, alpha = 1)
...
sf_map+stat_binhex(aes(x =X,y =Y, color=status),
                   bins =60, data = suicide)+
  coord_cartesian()



sf_map+stat_binhex(aes(x =X,y =Y, color=status),
                   bins = 70, data = suicide, alpha = 1)+
  coord_cartesian()
...
sf_map+stat_binhex(aes(x =X,y =Y, color=status),
                   bins = 70, data = suicide, alpha = 1)+
  coord_cartesian () + scale_fill_gradient(low ="#FFFFFF",high ="#f03b20")
...
dat <- data.frame(statusID = suicide$statusID, downtownID = suicide$downtownID)
...
downtownModel <- ulam(
  alist(
    statusID ~ dbinom( 1 , p ) ,
    logit(p) <- b[downtownID],
    b[downtownID ] ~ dnorm( 0 , .5 )
  ) , data=dat, log_lik = TRUE)

precis(downtownModel)

#  mean   sd  5.5% 94.5% n_eff Rhat4
#a 0.01 0.38 -0.58  0.66    77     1
...
downtownModel4k <- ulam(
  alist(
    statusID ~ dbinom( 1 , p ) ,
        logit(p) <-b[downtownID],
        b[downtownID ] ~ dnorm( 0 , .5 )
     ) , data=dat, iter= 4000, log_lik = TRUE)

precis(downtownModel4k)
...
# probabilities
means<- inv_logit(p$mean)
means

?inv_logit
# extract samples from our model
samples <- extract.samples(downtownModel4k)
samples
...
names(samples) <- c("surroundings", "downtown")
head(samples)
...
samplesLong$probability <- inv_logit(samplesLong$probability)
...
ggplot() + geom_violin(data = samplesLong, aes(x =location , y = probability))+
  geom_jitter(data = samplesLong, aes(x = location, y = probability), 
              size = .2, width = .1, alpha = .3)+
  theme_tufte(base_size = 12) +
  geom_jitter(height = .05,  data = suicide,
...
sf_neighboorhood_summaries <- left_join(sf_neighborhoods,neighborhoods)
?left_join
#The mutating joins add columns from y to x, matching rows based on the keys,includes all rows in x
...
names(sf_neighboorhood_summaries)

ggplot(data=sf_neighboorhood_summaries,aes(fill = statusID))+ geom_sf()+ theme_tufte(base_size = 12)

ggplot(data=sf_neighboorhood_summaries,aes(fill = number_suicides ))+geom_sf()+ theme_tufte(base_size = 12)
...
ggplot(data=sf_neighboorhood_summaries,aes(fill =  statusID/number_suicides))+ 
  scale_fill_gradient('success rate', low ="skyblue",high ="darkorange")+ ggtitle("Success rate by neighbourhood in San Francisco")+ 
   geom_sf() + theme_void() + theme_tufte(base_size = 12)
...
neihgborhoodModel <- ulam(
  alist(
    statusID ~ dbinom( number_suicides , p ) ,
    logit(p) <-b[nhoodID],
    b[nhoodID] ~ dnorm( 0 , 1.5 )
  ) , data=neighborhoods, iter= 4000, log_lik = TRUE)


# do you remember what to use when the predictor was categorical?
pneigh <- precis(neihgborhoodModel , depth= 39 )
...
successRate <- inv_logit(pneigh$mean)
...
ggplot() + geom_pointrange(aes(x =neighborhood , y = successRate, ymin = low , ymax = high ))+
  coord_flip()+theme_tufte()


#google how to order this, order by size, add title

ggplot() + geom_pointrange(aes(x = reorder(neighborhood,  successRate, decreasing = TRUE), 
                               y = successRate, ymin = low , ymax = high ))+
  coord_flip()+theme_tufte() + ggtitle("Success rate by neighbourhood in San Francisco") 
